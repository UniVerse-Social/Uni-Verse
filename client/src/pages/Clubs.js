...
// Reuse existing Marketplace page content inside the Marketplace tab
...
const Subbar = styled.div`
  ...
  & > button.active{ border-color:#111; background:#111; color:#fff; }
`;
...
const ComposerBox = styled.div`border:1px solid #eee; border-radius:12px; padding:10px; background:#fafafa; margin-bottom:12px;`;
const TA = styled.textarea`width:100%; min-height:70px; border:none; outline:none; resize:vertical; background:transparent;`;
const Input = styled.input`width:100%; padding:8px; border:1px solid #eee; border-radius:8px; margin-bottom:8px;`;
...
function EventsPanel(){
  ...
  const canPost = !!user?.isAdmin; // Only admins can post

  const submit = async ()=>{
    if(!title.trim() || !text.trim()) return;
    setSending(true);
    try{
      await axios.post(`${API_BASE_URL}/api/events`, { authorId: user._id, title, text });
  ...
  const del = async (id)=>{
    ...
    await axios.delete(`${API_BASE_URL}/api/events/${id}`, { data: { userId: user._id } });
    load();
  };
      ...
      <Body $scroll>
        {canPost && (
          <ComposerBox>
            ...
            <TA placeholder="Share an update…" value={text} onChange={e=>setText(e.target.value)} />
            <div style={{display:'flex', justifyContent:'flex-end'}}>
              <Btn onClick={submit} disabled={sending}>Post</Btn>
            </div>
          </ComposerBox>
        )}

        {events.map(ev=>(
          <div key={ev._id} style={{background:'#fff', border:'1px solid #eee', borderRadius:12, padding:12, marginBottom:12}}>
            <div style={{display:'flex', alignItems:'center'}}>
              <div style={{fontWeight:700}}>{ev.title}</div>
              <div style={{marginLeft:'auto', color:'#777', fontSize:12}}>{new Date(ev.createdAt).toLocaleString()}</div>
            </div>
...
export default function Clubs(){
  ...
  // top sub-navbar tab: 'clubs' | 'events' | 'market'
  ...
  const isMember = useMemo(() => selected && (selected.members || []).map(String).includes(String(user._id)), [selected, user._id]);
  const amPresident = useMemo(() => selected && String(selected.president) === String(user._id), [selected, user._id]);

  const createClub = async () => {
    const name = prompt('Club name? (unique)');
    if (!name) return;
    const description = prompt('Short description (optional)') || '';
    const res = await axios.post(api.clubs, { userId: user._id, name, description });
    setSelected(res.data);
  ...
  const onPhotoSelected = async (e) => {
    ...
    await axios.put(`${api.clubs}/${selected._id}/photo`, { actorId: user._id, dataUrl });
    await loadClub(selected._id); refreshMine(); searchClubs();
  };
      ...
      {tab==='clubs' && (
        <Page>
            ...
            <Body $scroll>
              ...
              {myClubs.map(c => (
                <Row key={c._id} onClick={() => loadClub(c._id)}>
                  <div style={{fontSize:12, width:28, height:28, borderRadius:8, background:'#f3f3f3', display:'grid', placeItems:'center', overflow:'hidden'}}>
                    {c.profilePicture
                  ...
                  {String(c.president) === String(user._id) && <FaCrown title="President" color="#d4a417" />}
                </Row>
              ))}
            </Body>
              ...
              <div style={{marginTop:8}}>
                {explore.map(c => (
                  <Row key={c._id} onClick={() => loadClub(c._id)}>
                    ...
                    {!c.isMember && (
                      <Ghost onClick={(e)=>{ e.stopPropagation(); join(c._id); }}><FaUserPlus/> Join</Ghost>
                    )}
                  </Row>
                ))}
              </div>
              ...
              <>
                <Head>
                  <div style={{display:'flex', alignItems:'center', gap:10}}>
                    <div onClick={amPresident ? choosePhoto : undefined} title={amPresident ? 'Change club photo' : ''} style={{width:36, height:36, borderRadius:'50%', overflow:'hidden', background:'#f2f2f2', display:'grid', placeItems:'center', cursor: amPresident ? 'pointer' : 'default'}}>
                      {selected.profilePicture
                        ? <img src={selected.profilePicture} alt="" style={{width:'100%',height:'100%',objectFit:'cover'}}/>
                        : <span style={{fontWeight:700}}>{selected.name[0]?.toUpperCase()}</span>}
                    </div>
                    <div style={{fontWeight:700}}>{selected.name}</div>
                    {amPresident && <FaCrown title="You are the President" color="#d4a417" style={{marginLeft:6}}/>}
                  </div>
                  <div style={{marginLeft:'auto', display:'flex', gap:8}}>
                    {!isMember ? (
                      <Btn onClick={()=>join(selected._id)}>Join</Btn>
                    ) : (
                      <Ghost onClick={()=>leave(selected._id)}><FaSignOutAlt/> Leave</Ghost>
                    )}
                    <Ghost onClick={()=>setShowMembers(true)}>Members</Ghost>
                  </div>
                  <input ref={photoInput} type="file" accept="image/*" onChange={onPhotoSelected} style={{display:'none'}} />
                </Head>

                <Chips>
                  <Chip $active={active.type==='main'} onClick={()=>setActive({ type:'main', sideId:null, name:'Main' })}>Main</Chip>
                  {(selected.sideChannels || []).map(sc => (
                    <Chip key={sc._id} $active={active.type==='side' && String(active.sideId)===String(sc._id)}
                      onClick={()=>setActive({ type:'side', sideId: sc._id, name: sc.name })}>
                      {sc.name}
                    </Chip>
                  ))}
                  {amPresident && (
                    <Ghost style={{marginLeft:'auto'}} onClick={newSideChannel}><FaPlus/> New channel</Ghost>
                  )}
                </Chips>
                ...
                <Body $scroll>
                  ...
                  <div style={{marginTop:12}}>
                    {posts.map(p => <ClubPostCard key={p._id} post={p} refresh={loadPosts} />)}
          ...
          {selected && showMembers && (
            <MemberDrawer club={selected} me={user} onClose={async()=>{ setShowMembers(false); await loadClub(selected._id); }} />
          )}
        </Page>
      )}
