...
/** ====== Layout constants (mirror Chess / Jump) ====== */
const SIDE_W = 360;
const HEADER_H = 76;
const BOTTOM_GAP = 40;
...
const FIRE_CD_TICKS = Math.max(
  1,
  Math.round(FIRE_CD_S * SIM_HZ)
);
const INV_SPAWN_TICKS = Math.round(
  INV_SPAWN_S * SIM_HZ
);
...
function drawRock(ctx, r) {
  ...
  // smiley
  ctx.fillStyle = WHITE;
  const eyeOff = rad * 0.35,
    eyeR = Math.max(1, rad * 0.08);
  ctx.beginPath();
  ctx.arc(
    -eyeOff,
    -eyeOff * 0.5,
...
function createEngine(ctx, hooks = {}) {
  ...
  const currentTick = () =>
    Math.floor(
      (performance.now() - state.simStartPerf) /
        TICK_MS
    );
  ...
  function resetWorld(seed) {
    ...
    const spawn = (det) =>
      spawnRock(3, undefined, undefined, det);
    if (HOOKS.online) {
  ...
  function spawnRock(
    ...
    const rock = {
      ...
      size,
      spin: 0,
      seed: Math.floor(r.range(0, 1000)),
    };
    rocks.push(rock);
  }
  ...
  function scheduleFire(pIdx, t) {
    fireQueue[pIdx].add(t);
  }
  function immediateFire(pIdx) {
    ...
    shots.push({
      ...
      vx:
        p.vx +
        Math.cos(p.a) * BULLET_SPEED,
      vy:
        p.vy +
        Math.sin(p.a) * BULLET_SPEED,
  ...
  function deterministicSpawnForTick(tick) {
    ...
    const elapsedSec =
      (tick * TICK_MS) / 1000;
    const maxRocks = Math.min(
      14,
      6 + Math.floor(elapsedSec / 45) * 2
    );
    const spawnProb =
      0.22 +
      Math.min(
  ...
  function simTick() {
    ...
    if (fireQueue[0].has(tick)) {
      immediateFire(0);
      fireQueue[0].delete(tick);
    }
    if (fireQueue[1].has(tick)) {
      immediateFire(1);
      fireQueue[1].delete(tick);
    }

    // periodic deterministic spawns in online; light random in practice/attract
    if (HOOKS.online) {
      if (
        tick % state.spawnPeriodTicks === 0
      )
        deterministicSpawnForTick(tick);
    } else {
      if (
        tick % 15 === 0 &&
        rocks.length < 6 &&
    ...
    for (let p of players) {
      ...
      if (p.thrust) {
        p.vx +=
          Math.cos(p.a) * ACCEL_PER_TICK;
        p.vy +=
          Math.sin(p.a) * ACCEL_PER_TICK;
      }
      p.vx *= FRICTION_PER_TICK;
      p.vy *= FRICTION_PER_TICK;
      p.x += p.vx * FIXED_DT;
      p.y += p.vy * FIXED_DT;
      wrap(p);
  ...
  function renderFrame() {
    ...
    const targetTick = Math.floor(
      (now - state.simStartPerf) / TICK_MS
    );
    let steps = 0;
    while (
      state.lastSimTick < targetTick &&
      steps < 900
    ) {
      // hard cap to avoid spiral
      simTick();
      steps++;
    }
    ...
    if (HOOKS.attract) {
      ...
      ctx.fillText(
        "Pick a mode to start — Practice or Play Online",
        W / 2,
        H / 2 + 4
      );
  ...
  function start(seed) {
    resetWorld(seed);
    state.running = true;
    state.simStartPerf = performance.now();
    state.lastSimTick = -1;
    requestAnimationFrame(renderFrame);
  }
  function stop() {
  ...
  function setKey(which, down) {
    ...
    if (HOOKS.online) {
      ...
      if (code)
        HOOKS.onInput?.(code, currentTick());
    }
  }
...
const BoardPanel = styled(Panel)`
  ...
  max-height: calc(
    100vh - ${HEADER_H}px - ${BOTTOM_GAP}px
  );
  overflow: hidden;
...
export default function MeteorArena({
  ...
  const startAttractAI = useCallback(() => {
    ...
    aiRef.current = setInterval(() => {
      ...
      engineRef.current?.opponentInput(
        Math.random() < 0.5
          ? "L1"
          : "R1"
      );
  ...
  const stopAttractAI = useCallback(() => {
    clearInterval(aiRef.current);
    ++epochRef.current;
    engineRef.current?.resetP1Inputs();
  }, []);
  ...
  const finish = useCallback(
    (win, reason = "lives") => {
      ...
      const [s1, s2] =
        engineRef.current?.getScores?.() ||
        [0, 0];
      const t1 =
        s1 +
        (reason === "time"
          ? livesRef.current.p1 * 1000
          : 0);
      const t2 =
        s2 +
        (reason === "time"
          ? livesRef.current.p2 * 1000
          : 0);
      setResult({
  ...
  useEffect(() => {
    ...
    if (
      livesRef.current.p1 > 0 &&
      livesRef.current.p2 > 0
    ) {
      const [s1, s2] =
        engineRef.current?.getScores?.() ||
        [0, 0];
  ...
  function practice() {
    ...
    setMode("bot");
    setStatus(
      "Practice — 3:00 · ←/→ rotate · ↑ thrust · space shoot."
    );
  ...
  const connectSocket = useCallback(() => {
    ...
    s.on(
      ...
      ({
        ...
        engineRef.current?.setLocalIndex(
          you === "p1" ? 0 : 1
        );
        ...
        engineRef.current?.setAttract(
          false
        );
        engineRef.current?.start(seed);
        setStatus(
          "Match found — 3:00 · ←/→ rotate · ↑ thrust · space shoot."
        );
        startTimer();
      }
    );
    s.on("meteor:input", (msg) => {
      engineRef.current?.opponentInput(
        msg.action,
        msg.t
      );
    });
    s.on("meteor:lives", ({ lives }) =>
      setLives(lives)
    );
    s.on(
      "meteor:gameover",
  ...
  const Overlay = ({ result }) => {
    ...
    const reason =
      result.reason === "time"
        ? "Time expired — lives converted to points."
        : "All lives lost.";
    return (
      <div
        style={{
      ...
      >
        <div>
          ...
          >
            P1 Total:{" "}
            <b>{result.totals.p1}</b> • P2
            Total:{" "}
            <b>{result.totals.p2}</b>
          </div>
          {mode !== "online" && (
            <div
              style={{
  ...
  /** --- responsive board sizing like Chess/Jump --- */
  ...
  const opponentMeta =
    mode === "online"
      ? `Hearts: P1 ${lives.p1}/3 · P2 ${lives.p2}/3`
      : "Practice · local AI";
    ...
    <>
      ...
      <MobileTopBar>
        <DrawerButton
          ...
          aria-label="Open Meteor sidebar"
        >
          ➤
        </DrawerButton>
      ...
      {/* Drawer with GameSidebar (mobile stats, like Chess/Jump) */}
      {drawerOpen && (
        <DrawerBackdrop
          onClick={() =>
            setDrawerOpen(false)
