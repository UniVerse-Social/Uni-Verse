...
function createTetris(ctx, baseX, seed, hooks={}){
  ...
  while (nextQ.length < 5){
    ...
    nextQ.push( clone(SHAPES[ bag.pop() ]) );
  }

  function spawn(){
    if (!bag.length) bag = makeBag(rng);
    G.piece = nextQ.shift();
    nextQ.push( clone(SHAPES[ (bag.length?bag:bag=makeBag(rng)).pop() ]) );
    G.px = Math.floor((COLS - G.piece[0].length)/2);
    G.py = -1;
    if (collide(G.px,G.py,G.piece)) G.over = true;
  ...
  function merge(){
    for (let y=0;y<G.piece.length;y++)
      for (let x=0;x<G.piece[0].length;x++)
        if (G.piece[y][x] && G.py+y>=0) G.grid[G.py+y][G.px+x] = 1;
  }
  ...
  function left(){ if (!collide(G.px-1,G.py,G.piece)) G.px--; }
  function right(){ if (!collide(G.px+1,G.py,G.piece)) G.px++; }
  function rot(){ const r=rotCW(G.piece); if (!collide(G.px,G.py,r)) G.piece=r; }
  function soft(){ if (!collide(G.px,G.py+1,G.piece)) { G.py++; G.score += 1; H.onScore?.(G.score); G.fallCount = G.fallInterval; } else lock(); }
  function hardDrop(){
    let rows=0;
    while(!collide(G.px,G.py+1,G.piece)){ G.py++; rows++; }
    if (rows>0){ G.score += rows*2; H.onScore?.(G.score); }
    lock();
  }
  ...
  function drawBoard(){
    ...
    for(let y=0;y<ROWS;y++)
      for(let x=0;x<COLS;x++){
        ...
        if (G.grid[y][x]){
          ctx.fillStyle="#111";
          ctx.fillRect(baseX+x*SIZE+1, y*SIZE+1, SIZE-2, SIZE-2);
        }
      }
    if (G.piece && !G.over){
      ctx.fillStyle="#111";
      for(let y=0;y<G.piece.length;y++)
        for(let x=0;x<G.piece[0].length;x++)
          if (G.piece[y][x])
            ctx.fillRect(baseX+(G.px+x)*SIZE+1, (G.py+y)*SIZE+1, SIZE-2, SIZE-2);
    }
    // HUD
    ...
    ctx.strokeStyle="#cbd5e1"; ctx.strokeRect(bx,by,boxW,boxH);
  ...
  function tick(){
    ...
    G.fallCount--;
    if (G.fallCount <= 0){
      if (!collide(G.px,G.py+1,G.piece)) { G.py++; G.fallCount = G.fallInterval; }
      else { lock(); }
    }
    drawBoard();
  }
...
export default function TetrisArena(){
  ...
  // render loop (draws; simulation advances only on server ticks)
  function loop(){
    meRef.current?.drawBoard(); opRef.current?.drawBoard();
    rafRef.current = requestAnimationFrame(loop);
  }
  function clearLoop(){ cancelAnimationFrame(rafRef.current); }

  function finish(didWin){
    clearLoop(); setMode(null);
    ...
    if (!awardedRef.current){
      ...
      axios.post(`${API_BASE_URL}/api/games/result`, {
        userId: window.__USER__?._id, gameKey:"tetris", delta: didWin?6:-5, didWin: !!didWin
      }).catch(()=>{});
    }
  }
  ...
  useEffect(()=>{
    ...
    const kd = (e)=>{
      ...
      if (mode==="bot"){
        // local practice plays instantly
  ...
  function startPractice(){
    ...
    // dumb bot for practice
    const bot = setInterval(()=>{
      if (!opRef.current || opRef.current.g.over){ clearInterval(bot); const win = !meRef.current.g.over && (meRef.current.g.score >= (opRef.current?.g.score||0)); finish(win); return; }
      const r=Math.random();
