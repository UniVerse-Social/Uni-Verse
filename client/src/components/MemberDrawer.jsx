...
export default function MemberDrawer({ club, me, onClose }) {
  ...
  const loadMembers = useCallback(async () => {
    const r = await axios.get(
      `${API_BASE_URL}/api/clubs/${clubDoc._id}/members`
    );
    setMembers(r.data || []);
  }, [clubDoc._id]);
  ...
  const toggleMain = async (u) => {
    const inMain = (clubDoc.mainPosters || [])
      .map(String)
      .includes(String(u._id));
    await axios.put(`${API_BASE_URL}/api/clubs/${clubDoc._id}/allow-main`, {
      actorId: me._id,
      targetId: u._id,
      allow: !inMain,
    });
  ...
  const transfer = async (u) => {
    ...
    await axios.put(`${API_BASE_URL}/api/clubs/${clubDoc._id}/transfer`, {
      actorId: me._id,
      targetId: u._id,
    });
    await refreshClub();
  ...
  // Side channels …
  const newSide = async () => {
    const name = prompt('New side channel name?');
    if (!name) return;
    await axios.post(
      `${API_BASE_URL}/api/clubs/${clubDoc._id}/side-channels`,
      { actorId: me._id, name }
    );
    await refreshClub();
  };
  ...
  const renameSide = async (sc) => {
    const name = prompt('Rename channel', sc.name);
    if (name === null) return;
    await axios.put(
      `${API_BASE_URL}/api/clubs/${clubDoc._id}/side-channels/${sc._id}/rename`,
      { actorId: me._id, name }
    );
    await refreshClub();
  };
  ...
  const setDirector = async (sc) => {
    const username = prompt(
      `Set director for "${sc.name}". Enter exact username from member list:`
    );
    if (!username) return;
    const m = members.find((x) => x.username === username);
    if (!m) return alert('No such member');
    await axios.put(
      `${API_BASE_URL}/api/clubs/${clubDoc._id}/side-channels/${sc._id}/director`,
      { actorId: me._id, targetId: m._id }
    );
    await refreshClub();
  };
  ...
  const deleteSide = async (sc) => {
    ...
    await axios.delete(
      `${API_BASE_URL}/api/clubs/${clubDoc._id}/side-channels/${sc._id}`,
      { data: { actorId: me._id } }
    );
    await refreshClub();
  };
        ...
        {amPresident && (
          <Section>
            ...
            <div style={{ marginTop: 8, display: 'grid', gap: 6 }}>
                ...
                >
                  <div>
                    ...
                    <Title>
                      ...
                      {sc.director
                        ? members.find(
                            (m) => String(m._id) === String(sc.director)
                          )?.username || 'Member'
                        : '—'}
                    </Title>
                  </div>
              ...
              <Ghost onClick={newSide} style={{ marginTop: 6 }}>
                + New side channel
              </Ghost>
            </div>
          </Section>
              ...
              <Row key={u._id}>
                ...
                {amPresident ? (
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    ...
                    <Ghost onClick={() => toggleMain(u)}>
                      {inMain ? 'Revoke Main' : 'Allow Main'}
                    </Ghost>
